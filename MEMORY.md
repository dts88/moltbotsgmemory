...
### FOIZ 库存周报（周二）
- **Cron**: `e650493f-56cb-4815-b24b-c12aad120191`，周二/三 10:00 SGT
- **脚本**: `scripts/foiz-inventory.mjs`，**API 详情见** `skills/foiz-inventory/SKILL.md`
- **输出**: Telegram 市场数据推送 topic（-1003727952836, threadId=1881）

### 新加坡库存周报（周四）
- **Cron**: `22f865a5-a942-4a11-98f0-5af85cea6d5f`，周四 14:00 SGT
- **脚本**: `scripts/sg-inventory.mjs`（Platts News Insights API 标题解析）
- **输出**: Telegram 市场数据推送 topic（-1003727952836, threadId=1881）

### Polymarket US x Iran Ceasefire Monitor
- **频率**: 每天 09:00 + 18:00 SGT（2026-03-30 改版）
- **模型**: Sonnet
- **Cron 09:00**: `81d24f9a-1ef2-4db5-93c8-4b29c947a07d`
- **Cron 18:00**: `1e1216d8-138a-4243-a126-70797ed8f89a`
- **脚本**: `scripts/polymarket-monitor.mjs`
- **输出**: WhatsApp +6592716786
- **内容**: US x Iran ceasefire by...? 全系列合约停火概率（slug: us-x-iran-ceasefire-by）
- **旧 cron 已删除**: `52d9ac0e-f7f0-4d06-b186-b51e5d1d3d4b`（US Strikes Iran 已结算YES）
...
### Kpler Hormuz Crossings Monitor
- **内容**: Kpler 霍尔木兹海峡过境数据（xlsx）
- **频率**: 每 8 小时
- **Cron**: `7eab20e0-52a3-483b-bfb4-6378023524d1`
- **脚本**: `scripts/kpler-hormuz-monitor.mjs`
- **数据页**: https://help.kpler.com/en/articles/14012671-strait-of-hormuz-transit-identification
- **存储**: `reports/hormuz/`（只保留最新一个xlsx）
- **输出**: 有更新时发 WhatsApp +6592716786 提醒 + 自动入知识库

### Thai News Monitor (Sun 803963798)
- **内容**: Bangkok Post + Thaiger + Thai PBS + Nation TH 每小时监控
- **关键词**: AIS/ADVANC, Gulf Development, True Corp + 泰国宏观/电信
- **Cron**: `3f45e0b2-92a9-4a5c-8354-ae878a72cc49`，每小时
- **脚本**: `scripts/thai-news-monitor.mjs`
- **去重缓存**: `.config/thai-news-monitor/seen.json`
- **输出**: Telegram -1003526235110 topic:7
- **排除词**: 社会/犯罪/生活类（scam, police, accident, tourist, weather, sport 等）
- **授权用户**: Sun (+803963798) 创建，属于小龙虾助手群组 topic:7
...
## 💬 Telegram Topics (群组 -1003727952836)

| Topic | threadId | 用途 | 推送任务 |
|-------|----------|------|---------|
| 📰 市场日报 | **2008** | 每日市场报告 | — |
| 📊 市场数据推送 | **1881** | 周度数据推送 | EIA周报、新加坡库存、FOIZ库存 |
| MOC | **2** | Dubai MOC 日报推送 | Dubai MOC 日报（2026-03-30 迁回）|
| 系统管理 | 7 | 运维/配置对话 | — |
| 邮箱 | 22 | Gmail 管理 | — |

*Topic 迁移时间: 2026-03-30*
...
- `e650493f-56cb-4815-b24b-c12aad120191` - FOIZ 库存周报 (周二三 10:00 SGT，Telegram 市场数据推送 topic，脚本 foiz-inventory.mjs)
- `81d24f9a-1ef2-4db5-93c8-4b29c947a07d` - Polymarket US-Iran Ceasefire Monitor (每天09:00 SGT，2026-03-30 改版)
- `1e1216d8-138a-4243-a126-70797ed8f89a` - Polymarket US-Iran Ceasefire Monitor (每天18:00 SGT，2026-03-30 新增)
- `edff591f-368d-4e82-9a59-d6a3d03426dd` - Hormuz Monitor ⛔已禁用 (2026-03-10，合并至Polymarket Monitor)
- `22f865a5-a942-4a11-98f0-5af85cea6d5f` - 新加坡库存周报 (周四14:00 SGT，Telegram 市场数据推送 topic，脚本 sg-inventory.mjs)
- `b97e0428-3da9-464e-8e88-2d5033f46e65` - Dubai MOC Daily Report (周一至周五 16:36 SGT)
  - ⚠️ **已知问题**: 16:36 SGT 触发，FluxOfficials 推文约 16:38 才发 → assessment 为 null，需改为 16:45 SGT
...
- `7eab20e0-52a3-483b-bfb4-6378023524d1` - Kpler Hormuz Crossings Monitor ⛔已禁用 (2026-03-31，Kpler 页面改版移除xlsx下载，改为需要API凭证)
- `9ddf0631-1725-4af9-93cb-8a4c911a815a` - ICE Brent Daily Report ⛔已禁用 (2026-03-31，Tianshu 取消)

### Dubai MOC Premium 大崩溃（2026-03-24/25/26）
- 3/24: Premium $61.09 → 3/25: $51.73 → 3/26: $17.11（3天 -$44）
- 根本原因：Trump 美伊谈判预期，停火乐观情绪急剧升温
- 运行月均值 3/26 收于 $40.40（峰值时 ~$60+）
- Murban 是唯一可绕行交割级别，危机期间升水显著高于 Oman
- TOTSA 3月底仍是唯一买家，每天吃下全部 declarations

### MTBE FOB Singapore 符号状态（2026-03-27 调查）
- **eWindow 无 ASIA MTBE 市场**（3/26 全量扫描确认）
- **AAYFG00 不是 MTBE 价格**（值固定 -10 或 -13，应为 octane 参数）
- **AACM 系列**（如 AACMG00）存在但无订阅权限（unsubscribed）
- **EU MTBE (ARA Barges)**: $1,153/mt（3/26）可作参考
- **Platts 方法论（2025-01改版）**: MTBE = 92 RON MOPS Strip + differential，在 eWindow 竞价
- 正确符号待确认（需 Tianshu 提供 APAG Marketscan 里的代码）
...
**主要沟通渠道**: Telegram MOC topic (-1003727952836, threadId: **2**) ← 2026-03-30 迁回 topic:2（原 threadId:2008 已停用）
...
### ⚠️ Cron 时间与数据验证问题（2026-03-31 严重教训）
- **问题 1**: cron 16:36 SGT 触发，FluxOfficials 推文 16:38 才发 → assessment/premium 为 null
- **问题 2**: 3/27 报告用了 3/26 的 FluxOfficials 数据（存档写入错误），直接发出错误报告
- **根本原因**: 
  - 数据抓取与推文发布时间有竞争条件
  - 存档数据写入前未验证日期戳
- **修复方案**:
  - 将 cron 时间改为 **16:45 SGT**（待执行）
  - **生成报告前必须验证数据日期戳**，避免用前一天的缓存数据
- **教训**: 自动化任务涉及外部数据源时，必须设计时间缓冲 + 数据验证机制
...
### 历史事件回顾：2026-03 Dubai MOC 极端行情
- **3/24 单日暴跌 -$11.73**：Trump 美伊谈判信号，Brent 跌破 $100/bbl
- **3/25 继续暴跌 -$14.5**：物理溢价从 $61 → $51，买盘几乎消失
- **3/26 崩溃延续 -$29**：convergence price $113.04，Premium 跌至 $17
- 整个3月 TOTSA 单一买家，累计 39 cargoes（3月前10个交易日）
- 危机期间 Apr/May 价差最高超 $12/bbl（超级 backwardation）
...
## 🛢️ Dubai Cracking Margin (DBSCM00) 分析总结 (2026-03-30 最终版 ✅)

### 官方公式（Platts 官方确认，2026-03-30）
```
DBSCM00 = DBSCY00 − TDDCQ00 − PCAAT00
```
**9天全部误差 ≤ 0.01 $/bbl，完全是浮点精度**

### 核心符号
| 符号 | 含义 | 类型 |
|------|------|------|
| DBSCM00 | Dubai Singapore Cracking **Margin** | $/bbl |
| DBSCN00 | Dubai Singapore Cracking **Netback** = DBSCY00 − TDDCQ00 | $/bbl |
| DBSCY00 | **GPW**（Gross Product Worth）= Σ(yield × product price) | $/bbl |
| TDDCQ00 | **运费**（Platts 计算，bate=u）| **$/bbl 直接用，无需换算** |
...
### 产品符号完整对照表（Tianshu 2026-03-30 官方确认）

| 产品 | Symbol | 单位 | $/bbl换算 |
|------|--------|------|----------|
| Propane Ref CFR Japan | AAVAK00 | $/mt | ÷12.480 |
| Butane Ref CFR Japan | AAVAN00 | $/mt | ÷10.749 |
| Naphtha FOB Spore | PAAAP00 | $/bbl | 直接 |
| Gasoline 92 RON FOB Spore | PGAEY00 | $/bbl | 直接 |
| Gasoline 97 RON FOB Spore | PGAMS00 | $/bbl | 直接 |
| Jet Kero FOB Spore | PJABF00 | $/bbl | 直接 |
| GO 10ppm FOB Spore | AAOVC00 | $/bbl | 直接 |
| GO 500ppm FOB Spore | AAPPF00 | $/bbl | 直接 |
| GO 2500ppm FOB Spore | AACUE00 | $/bbl | 直接 |
| FO 180 2.0%S FOB Spore | PUAXS00 | $/mt | ÷6.35 |
| FO 180 3.5%S FOB Spore | PUADV00 | $/mt | ÷6.35 |
| FO 380 3.5%S FOB Spore | PPXDK00 | $/mt | ÷6.35 |
| LSWR → Marine Fuel 0.5% FOB Spore | AMFSA00 | $/mt | ÷6.80 **+$1/b** (Platts官方替代) |
| Petcoke FOB US Gulf 6.5%S | 未找到 | $/mt | 约7% 产率但价格低，贡献≈0 |

**LSWR说明**: FOB Indonesia LSWR 2019-04-01停止报价。Platts推荐：AMFSA00 + $1/b = Mixed/Cracked LSWR替代价。

### 产率回归最终版（无约束NNLS，307天，RMSE=0.179 $/bbl ✅ 2026-03-30）

| 产品 | Symbol | 产率 | 单位换算 |
|------|--------|------|---------|
| Propane Ref CFR Japan | AAVAK00 | 2.15% | $/mt ÷12.480 |
| Butane Ref CFR Japan | AAVAN00 | 3.67% | $/mt ÷10.749 |
| Naphtha FOB Spore | PAAAP00 | 10.23% | $/bbl |
| 92 RON FOB Spore | PGAEY00 | 8.68% | $/bbl |
| 97 RON FOB Spore | PGAMS00 | 6.65% | $/bbl |
| Jet Kero FOB Spore | PJABF00 | 13.80% | $/bbl |
| GO 10ppm FOB Spore | AAOVC00 | 14.76% | $/bbl |
| GO 500ppm FOB Spore | AAPPF00 | 9.54% | $/bbl |
| GO 2500ppm FOB Spore | AACUE00 | 8.68% | $/bbl |
| FO 180 2.0%S FOB Spore | PUAXS00 | 2.59% | $/mt ÷6.35 |
| FO 180 3.5%S FOB Spore | PUADV00 | 3.33% | $/mt ÷6.35 |
| FO 380 3.5%S FOB Spore | PPXDK00 | 5.84% | $/mt ÷6.35 |
| LSWR → MF0.5% FOB Spore | AMFSA00 | 6.94% | $/mt ÷6.80 **+$1/b** |
| **合计** | | **96.87%** | |
| Petcoke + Loss | — | 3.13% | — |

**验证精度（最近10天，3/16-3/27）**：误差全部 < ±0.4 $/bbl ✅
**稳健产品**（跨数据集差异 < 0.5%）：92 RON(8.68%)、Jet(13.8%)、GO10(14.76%)、GO500(9.54%)、GO2500(8.68%)
**不稳定**：FO180_2S/3.5S 互相替代（总和 ~5.9% 稳定）；LSWR 全量6.94% vs 2026年9.88%

### 关键认知修正（2026-03-30 最终版）
- ✅ **LSWR 正确替代 = AMFSA00 + $1/b**（Platts官方方法论，÷6.80 bbl/mt）
  - LSWR Mixed/Cracked FOB Indonesia 已于 2019-04-01 停止报价
  - Platts官方推荐：AMFSA00（Marine Fuel 0.5% FOB Spore）+ $1/b
  - AALRX00 数据仍在更新，但已非 Platts GPW 计算所用
- ✅ **运费 = TDDCQ00**（$/bbl 直接用，bate=u）
- ✅ **GPW 公式验证完毕**: DBSCM00 = DBSCY00 - TDDCQ00 - PCAAT00（9天误差≤0.01）
...
### 历史验证数据（3/17~3/27，9天全✅）
| 日期 | DBSCY00 | TDDCQ00 | PCAAT00 | DBSCM00 | 误差 |
|------|---------|---------|---------|---------|------|
| 3/17 | 156.25 | 8.47 | 157.66 | -9.88 | 0.00 |
| 3/18 | 156.56 | 8.12 | 155.55 | -7.11 | 0.00 |
| 3/19 | 174.08 | 7.67 | 166.80 | -0.38 | 0.01 |
| 3/20 | 170.68 | 7.55 | 158.85 | +4.28 | 0.00 |
| 3/26 | 157.62 | 5.79 | 113.04 | +38.79 | 0.00 |
| 3/27 | 170.66 | 5.82 | 122.14 | +42.70 | 0.00 |

---

## 🔬 DBSCM00 产率回归分析（已归档，不再需要）

由于 DBSCY00（GPW）是独立符号，可直接获取，不再需要手动 Σ(yield × crack)。
产率回归的问题（多重共线性）**已成为历史遗留问题**，不影响实际计算。

回归估算产率仍作参考（了解各产品贡献），见上方产率表。

*2026-03-30 归档，官方公式已完全验证*
