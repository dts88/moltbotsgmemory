...
function formatEvent(event) {
  ...
  if (event.start?.dateTime) {
    ...
    const endTime = new Date(end).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false });
    timeStr = `${startTime}-${endTime}`;
  } else {
    timeStr = '全天';
  }
  
  return `${timeStr} | ${event.summary || '(无标题)'}${event.location ? ` @ ${event.location}` : ''}`;
}

// 格式化事件列表
function formatEvents(events, title = '日程') {
  ...
  try {
    switch (command) {
      ...
      case 'week': {
        ...
        console.log(formatEvents(events, `未来 ${days} 天日程`));
        break;
      }
