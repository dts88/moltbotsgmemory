...
STATE_FILE=".config/crude-daily-snippet-weixin-state.json"

{
  echo "[$(date -Is)] starting crude daily snippet WeChat send"

  flock -n /tmp/clawd-crude-daily-snippet-weixin.lock bash -lc '
    set -euo pipefail
...
'"'"' "$state_file")"
    ...
    node scripts/weixin-send-text.mjs < "$tmp"
...
} >> logs/crude-daily-snippet-weixin.log 2>&1
